...
import { deepAssign, findIndex, isNumber, isArray, isDate } from '../../common/helper/utils';
...
createDatePickerComponent({
    ...
    methods: {
        onColumnChange(e) {
            ...
            if (newIndex === oldSelected[column])
                return;
        ...
        range(type, min, max, fatherIsMin, fatherIsMax, year) {
            ...
            for (let i = min; i <= max; i++) {
                ...
                if (fatherIsMin && i === min)
                    object.isMin = true;
                if (fatherIsMax && i === max)
                    object.isMax = true;
                if (year)
                    object.year = year;
                arr.push(object);
        ...
        generateData(i, count) {
            ...
            while (count < this.columnCount && i < 6) {
                ...
                const index = findIndex(list[count], (item) => {
        ...
        updateValueArray(i, newIndex) {
            const [valArray, minArray, maxArray, startIndex] = [this.valueArray, this.minArray, this.maxArray, +this.startIndex];
